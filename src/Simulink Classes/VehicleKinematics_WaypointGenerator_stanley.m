...
%             if obj.drivingMode == 1 %left lane switch
%                [position,orientation] = obj.leftLaneSwitch(car,speed);
%             end
        ...
        function [position,orientation] = leftLaneSwitch(obj,car,speed)
            obj.generateLeftSwitchWaypoints(car.dynamics.position, speed, car.pathInfo.currentTrajectory);
            ...
            if norm(checkPoint-Destination)< 4 % Error tolerance value TODO: check lower numbers
        ...
        function generateLeftSwitchWaypoints(obj,vehiclePosition, speed, currentTrajectory)
            route_Vector = currentTrajectory(2,:)-currentTrajectory(1,:);
            local_route_Vector_i = route_Vector/norm(route_Vector).*[1 1 -1];
            local_route_Vector_i_angle = acos(dot(local_route_Vector_i,[1 0 0]));
            if (local_route_Vector_i(3)<0)
                local_route_Vector_i_angle=2*pi-local_route_Vector_i_angle;
            end
            ...
            local_side_vector = [cos(local_route_Vector_i_angle+pi/2) 0 sin(local_route_Vector_i_angle+pi/2)];
            local_WP_start_point = local_WP_start_point+local_side_vector*lane_offset;
            obj.referencePose = [local_WP_start_point(1); local_WP_start_point(3); local_route_Vector_i_angle*180/pi];
        end
        function [position, orientation] = move_straight(obj,car,speed,Destination)
%             if obj.drivingMode == 1
%                 lane_offset = 20;
%             else
%                 lane_offset = 0;
%             end
        ...
        function [position, orientation] = rotate_left(obj ,car, speed, rotation_point,rotation_angle,Destination)
            ...
            if  norm(checkPoint - Destination) < 4 % TODO Check the value
                lastWaypoint = car.map.get_waypoint_from_coordinates(Destination);
...
%             end
        ...
        function generateLeftRotationWaypoints(obj,vehiclePosition, speed, currentTrajectory, Destination)
            ...
            target_point_P = [r,local_start_angle];
            obj.referencePose = [local_rotation_center(1)+r*cos(target_point_P(2)); local_rotation_center(3)+r*sin(target_point_P(2)); local_start_angle*180/pi+90];
%             for i = 1:1:length(obj.referenceWaypoints)
...
%             end
        ...
        function generateRightRotationWaypoints(obj,vehiclePosition, speed, currentTrajectory, Destination)
            ...
            target_point_P = [r,mod(l_angle,-2*pi)];
            obj.referencePose = [local_rotation_center(1)+r*cos(target_point_P(2)); local_rotation_center(3)+r*sin(target_point_P(2)); target_point_P(2)*180/pi-90];
%             for i = 1:1:length(obj.referenceWaypoints)
...
%             end
        ...
        function [x,y] = get_vehicle_checkpoint(obj,car)
            ...
            checkPoint_offset = 3;%determine the offset distance between vehicle CoG and checkpoint
