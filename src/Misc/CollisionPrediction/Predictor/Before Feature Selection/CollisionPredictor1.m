classdef CollisionPredictor1 < matlab.System & matlab.system.mixin.Propagates ...
    ...
    methods(Access = protected)
        ...
        function stepImpl(obj)
            ...
            if get_param(obj.modelName,'SimulationTime') == t
                    ...
                    elseif obj.Vehicles(i).pathInfo.currentRoute == 4 &&  obj.Vehicles(i).dynamics.speed ~= 0 %crossway point9A
                        for k = 1:10
                            if (obj.Vehicles(k).pathInfo.currentRoute == 47 || obj.Vehicles(k).pathInfo.currentRoute == 6 ...
                                || obj.Vehicles(k).pathInfo.currentRoute == 82 || obj.Vehicles(k).pathInfo.currentRoute == 25 )...
                                    &&  obj.Vehicles(k).dynamics.speed ~= 0
                                Speed_A = obj.Vehicles(i).dynamics.speed;
                                Position_A = obj.Vehicles(i).dynamics.position;
                                Speed_B = obj.Vehicles(k).dynamics.speed;
                ...
                if H ~= 0;
                    [Predict,~] = predict(Version3_1RF.ClassificationEnsemble,Data0(:,1:12));
                    x = 1;
                    y= (1:10);
                    for z = 1:H
                        if v(Data0.A(z)) ~= 1
